...
void init_scheduler_vars()
{
	...
	counter = 9; // counter value is 0-15
	high_threshold = 12;
	low_threshold = 6;
	current  = 0;
		...
		if(drain_writes[channel])
		{
			...
			LL_FOREACH(write_queue_head[channel], wr_ptr)
			{
				if(wr_ptr->command_issuable)
				{
					...
					if( prev_row.channel==wr_ptr->dram_addr.channel && prev_row.rank==wr_ptr->dram_addr.rank && prev_row.bank==wr_ptr->dram_addr.bank&& prev_row.row==wr_ptr->dram_addr.row)
					{
						counter++; // increment row if page hit
		...
		if(!drain_writes[channel])
		{
			LL_FOREACH(read_queue_head[channel],rd_ptr)
			{
				if(rd_ptr->command_issuable)
				{
					...
					if(prev_row.channel==rd_ptr->dram_addr.channel && prev_row.rank==rd_ptr->dram_addr.rank && prev_row.bank==rd_ptr->dram_addr.bank&& prev_row.row==rd_ptr->dram_addr.row)
					{
						counter++; // increment row if page hit
						if(counter>high_threshold)
						{
		...
		if(!drain_writes[channel])
		{
			LL_FOREACH(read_queue_head[channel],rd_ptr)
			{
				if(rd_ptr->command_issuable)
				{
					...
					{
						counter--;// increment counter when page miss
						prev_row.channel = rd_ptr->dram_addr.channel; // prev_row = current_row
						prev_row.rank = rd_ptr->dram_addr.rank;
						prev_row.bank = rd_ptr->dram_addr.bank;
						prev_row.row = rd_ptr->dram_addr.row;
